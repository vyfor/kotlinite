...
class KotlinDocumentManager(
  ...
  override fun completion(params: CompletionParams) = scope
         .async<Either<List<CompletionItem>, CompletionList>> {
            Either.forLeft(
              session?.project?.let { project ->
                fileRegistry.fileInfos[params.textDocument.uri.toURIPath()]?.second?.let { info ->
                  info.index?.let { index ->
                    index.declarations.filterNot {
                      decl -> decl.name == null
                    }.map { decl ->
  ...
  override fun didOpen(params: DidOpenTextDocumentParams) {
    ...
    fileRegistry.openFile(
      params.textDocument.uri.toURIPath(),
  ...
  override fun didChange(params: DidChangeTextDocumentParams) {
    ...
    val file = fileRegistry.openFiles[path]
      ?: run {
        fileRegistry.openFile(path, params.contentChanges[0].text, params.textDocument.version)
        return
      }

    FileDocumentManager.getInstance().getDocument(file)?.setText(params.contentChanges[0].text) ?: run {
